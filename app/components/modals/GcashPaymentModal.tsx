...
const GcashPaymentModal: React.FC<GcashPaymentModalProps> = ({
      ...
      const fetchQRCode = async () => {
        try {
          const response = await fetch(
            "/api/admin/subscriptions/payments/qrcode"
          );
          if (!response.ok) {
  ...
  let bodyContent = (
    <div className="p-4">
      <Heading
        ...
        subTitle="Review the details of your payment"
      />
      <div className="mt-4">
        <p>
  ...
  if (step === STEPS.QR_CODE) {
    bodyContent = (
      <div className="p-4 mt-24">
        <Heading
          title="Step 2: GCash QR Code"
          subTitle="Scan the QR code below to make your payment"
        />
        ...
        <p>
          GCash Information:{" "}
          <strong>{qrCodeData?.accountNumber || "Loading..."}</strong>
        </p>
        <p>
          Account Name:{" "}
          <strong>{qrCodeData?.accountName || "Loading..."}</strong>
        </p>
      </div>
    );
  }
  ...
  if (step === STEPS.UPLOAD) {
    bodyContent = (
      <div className="p-4">
        <Heading
          title="Step 3: Upload Payment Receipt"
