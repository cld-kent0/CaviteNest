...
const GcashPaymentDetailsModal: React.FC<GcashPaymentDetailsModalProps> = ({
  ...
  const [newReferenceNo, setNewReferenceNo] = useState<string>(
  ...
  async function handleApprove() {
    ...
    try {
      const response = await fetch(
        `/api/admin/subscriptions/payments/gcash/${payment.id}/approve`,
        {
          method: "POST",
          headers: {
  ...
  const handleDecline = async () => {
    try {
      const response = await fetch(
        `/api/admin/subscriptions/payments/gcash/${payment.id}/decline`,
        {
          method: "POST",
        }
      );
  ...
  const handleUpdatePlan = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/subscriptions/payments/gcash/${payment.id}/upgrade-plan`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="space-y-4">
      {step === 0 && (
        <div>
          ...
          <p>
            <strong>Customer ID:</strong> {payment.user.customerId}
          </p>
          <p>
            <strong>Reference Number:</strong> {payment.referenceNo || "N/A"}
          </p>{" "}
          ...
          <p>
            <strong>Billing Period:</strong> {payment.billingPeriod}
          </p>
          <p>
        ...
        <>
          ...
          {payment.status === "PENDING" && payment.user.customerId ? (
            ...
            <button
              ...
              className={`w-1/3 p-2 rounded-lg text-white ${
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center bg-neutral-800/70 ${modalClasses} transition-all duration-300`}
    >
      <div className="relative w-full h-full mx-auto my-6 md:w-4/6 lg:w-3/6 xl:w-2/5 lg:h-auto md:h-auto">
        <div className="relative flex flex-col w-full h-full bg-white border-0 rounded-lg shadow-lg">
          ...
          {/* BODY */}
          <div className="relative flex-auto p-6">{bodyContent}</div>
