    ...
    try {
        switch (event.type) {
            case "checkout.session.completed":
                ...
                if (customerDetails?.email) {
                    ...
                    for (const item of lineItems) {
                        ...
                        let period: "none" | "yearly" | "quarterly" = "none";
                        ...
                        if (isSubscription) {
                            ...
                            if (priceId === process.env.STRIPE_YEARLY_PREMIUM_PRICE_ID) {
                                ...
                                plan = "premium";
                                period = "yearly";
                            } else if (priceId === process.env.STRIPE_QUARTERLY_PREMIUM_PRICE_ID) {
                                endDate.setMonth(endDate.getMonth() + 3);
                                plan = "premium";
                                period = "quarterly";
                            } else if (priceId === process.env.STRIPE_YEARLY_BUSINESS_PRICE_ID) {
                                endDate.setFullYear(endDate.getFullYear() + 1);
                                plan = "business";
                                period = "yearly";
                            } else if (priceId === process.env.STRIPE_QUARTERLY_BUSINESS_PRICE_ID) {
                                endDate.setMonth(endDate.getMonth() + 3);
                                plan = "business";
                                period = "quarterly";
                            } else {
            ...
            case "customer.subscription.deleted": {
                ...
                const user = await prisma.user.findFirst({ //dapat findUnique to but dahil sa error gawin ko muna temporary na hindi - dars
